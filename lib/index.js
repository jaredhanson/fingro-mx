var uri = require('url')
  , dns = require('dns')
  , NoSupportError = require('./errors/nosupporterror')
...
exports = module.exports = function(exchanges) {
  ...
  plugin.resolveServices = function(identifier, type, cb) {
    ...
    if (url.protocol != 'acct:') {
      ...
      return cb(new NoSupportError('Scheme not supported: ' + url.protocol));
    }
    ...
    dns.resolve(url.hostname, 'MX', function (err, records) {
      ...
      // exchanges.
      return cb(new NotFoundError('Unknown exchange: ' + url.hostname));
    });
  }
